...
void DrawIndexed(const Context& context, const State& state, const Pm4::IndexedDraw& draw, const ShaderRecompiler::RecompileResult& vertex, const ShaderRecompiler::RecompileResult& fragment) {
    ...
    ShaderResources resources(context, vertex, fragment, state.color, draw.indexAddress, static_cast<std::size_t>(indexBytes));
    Pipeline pipeline(context, state, target, resources, vertex, fragment);
    ...
    context.Function<PFN_vkCmdPipelineBarrier>("vkCmdPipelineBarrier")(commands, VK_PIPELINE_STAGE_HOST_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT | VK_PIPELINE_STAGE_VERTEX_INPUT_BIT | VK_PIPELINE_STAGE_VERTEX_SHADER_BIT | VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0, 1, &upload, 0, nullptr, 0, nullptr);
    ...
    pipeline.PushConstants(commands, vertex, fragment);
    ...
    context.Function<PFN_vkCmdPipelineBarrier>("vkCmdPipelineBarrier")(commands, VK_PIPELINE_STAGE_TRANSFER_BIT | VK_PIPELINE_STAGE_VERTEX_SHADER_BIT | VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, VK_PIPELINE_STAGE_HOST_BIT, 0, 1, &download, 0, nullptr, 0, nullptr);
    batch.SubmitAndWait();
